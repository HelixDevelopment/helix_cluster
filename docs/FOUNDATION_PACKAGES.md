...
| `raft` | Embedded `github.com/hashicorp/raft` consensus: a `KVFSM` replicated key/value state machine (`Apply`/`Snapshot`/`Restore`), a `Node` wrapper (`Apply`/`Leader`/`IsLeader`/`State`/`Shutdown`) and `NewInmemCluster` for N-node bootstrap over hashicorp/raft's in-memory transport+store. Real 3-node kill-the-leader integration test: a new leader is elected and committed log entries stay intact on survivors, `-race` clean and non-flaky (HXC-1216). Beyond the in-mem path, three production primitives over the same `KVFSM`/`Node`/`Cluster` contract: (1) a real TCP **network** transport (`tcp.go`) — `tcpStreamLayer` over `net.ListenTCP` (loopback ephemeral port) wrapped in `raft.NewNetworkTransportWithConfig`, with `NewNetworkCluster(ids...)` standing up an N-node cluster whose RequestVote/AppendEntries/InstallSnapshot RPCs travel over genuine kernel sockets (leak-safe partial-failure cleanup; `NetTransport`/`LocalTCPAddr` accessors), stores still in-mem (HXC-974); (2) real on-disk **persistence** (`store.go`) — `NewPersistentNode(id,dataDir)` backed by a `hashicorp/raft-boltdb/v2` `BoltStore` (one bbolt file as log+stable store) plus a `FileSnapshotStore`, so a Shutdown→re-open of the same dir recovers all committed entries from disk WITHOUT caller re-apply (reopen detected → no re-bootstrap; `WaitForLeader` issues a `Barrier` so recovery is fully applied before return), in-mem transport (HXC-976); (3) a combined **persistent+networked** production node (`persistnet.go`) — `NewPersistentNetworkNode` / `NewPersistentNetworkCluster(ids,dataDirs)` composing the real TCP transport AND the real BoltDB+snapshot persistence into one node (restart→recover-from-disk→rejoin over a freshly bound port), reusing `openTCPTransport`/`openPersistentStores` so the two halves can't drift (Task I5). For multi-**process** clusters, `NewPersistentNetworkNodeAt(id,dataDir,bindAddr,base,servers)` binds a caller-specified fixed TCP address with an explicit server set (first boot bootstraps; reopen recovers-from-disk without re-bootstrap), via the single shared `openTCPTransportAt` (no TCP-wiring duplication). This primitive backs the `helix-raftd` daemon (`cmd/helix-raftd`): one persistent+networked node per OS process with an HTTP admin API (`/status`, `/get`, leader-only `/put` via raft `Apply` — a follower returns 421 + the real leader address, so no split-brain), proven by a real 3-process E2E that survives a leader-process `SIGKILL` and a full-cluster restart-from-disk where every value re-served could only have come from on-disk BoltDB (HXC-982). |
| `raftleader` | Real Raft-backed leader-election service adapting `raft` to a `leader`-shaped contract: `RaftElection` (`IsLeader`/`LeaderID`/`Resign` via a real `LeadershipTransfer`, `Apply` leader-gated, `Campaign`/`WaitForLeadership`, `LeadershipChanges` sourced from hashicorp/raft's `LeaderCh`, `ErrNotLeader`). 3-node test: exactly-one-leader, follower write-gating, kill-leader re-election, committed state survives failover (HXC-953). Closes the gap where `leader` was not real Raft; `leader` left untouched. |
