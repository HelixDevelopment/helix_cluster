...
| `raft` | Embedded `github.com/hashicorp/raft` consensus: a `KVFSM` replicated key/value state machine (`Apply`/`Snapshot`/`Restore`), a `Node` wrapper (`Apply`/`Leader`/`IsLeader`/`State`/`Shutdown`) over in-memory transport+store, and `NewInmemCluster` for N-node bootstrap. Real 3-node kill-the-leader integration test: a new leader is elected and committed log entries stay intact on survivors, `-race` clean and non-flaky (HXC-1216). |
